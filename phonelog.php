<?php
    ...
    if(isset($_POST['submit']))
    {
        ...
        if(iconv_strlen($_POST['readers_phone'], 'UTF-8') < 5 or iconv_strlen($_POST['readers_phone'], 'UTF-8') > 25)
        {
            $err = " ";
            echo "Номер телефона пользователя может быть длиной от 5 до 25 символов!<br>";
        }
        if(count($err) == 0)
        {
            $query = mysqli_query($link, "SELECT users_id, readers_phone FROM Users WHERE users_login = '".mysqli_real_escape_string($link, $_POST['users_login'])."' LIMIT 1");
            ...
            {
                if($data['readers_phone'] === $_POST['readers_phone'])
                {
                ...
                {
                    echo "Введён неправильно номер телефона пользователя";
                }
            }
        }
        else
